	...
	<body id="kt_body" style="background-image: url(<?php echo base_url(); ?>assets/template/metronic/media/bg/header.jpg)" class="quick-panel-right demo-panel-right offcanvas-right header-fixed subheader-enabled page-loading">
								...
								<div class="d-flex align-items-center flex-wrap mr-2">
									<div class="d-flex flex-column">
										...
										<div class="d-flex align-items-center font-weight-bold my-2">
                                            ...
                                            <!--begin::Item-->
											<span class="label label-dot label-sm bg-white opacity-75 mx-3"></span>
											<a href="" class="text-dark text-hover-white opacity-75 hover-opacity-100">Level Pekerjaan</a>
											<!--end::Item-->
										</div>
										<!--end::Breadcrumb-->
									</div>
                                ...
                                <div class="card card-custom">
                                    <div class="card-header flex-wrap border-0 pt-6 pb-0">
                                        <div class="card-title">
                                            <h3 class="card-label">Daftar Level Pekerjaan
        ...
        <div class="modal fade" id="exampleModalLong" data-backdrop="static" tabindex="-1" role="dialog" aria-labelledby="staticBackdrop" aria-hidden="true">
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    ...
                    <form method="POST" action="" id="form-add-job_level">
                        <input type="hidden" name="<?php echo $this->security->get_csrf_token_name(); ?>" value="<?php echo $this->security->get_csrf_hash(); ?>" id="csrf_token" />
                        <div class="modal-body">
                            <div class="form-group">
                                <label>Nama Level Pekerjaan</label>
                                <input type="text" name="job_level_name" id="add_job_level_name" class="form-control">
                            </div>
                            <div class="form-group">
                                <label for="exampleSelect1">Status</label>
                                <select class="form-control" name="job_level_status" id="add_job_level_status">
        ...
        <div class="modal fade" id="modal-edit" data-backdrop="static" tabindex="-1" role="dialog" aria-labelledby="staticBackdrop" aria-hidden="true">
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    ...
                    <form method="POST" action="" id="form-edit-organization">
                        <input type="hidden" name="<?php echo $this->security->get_csrf_token_name(); ?>" value="<?php echo $this->security->get_csrf_hash(); ?>" id="csrf_token" />
                        <div class="modal-body">
                            <div class="form-group">
                                <label>Nama Level Pekerjaan</label>
                                <input type="text" name="job_level_name" id="edit_job_level_name" class="form-control">
                                <input type="hidden" name="job_level_id" id="edit_job_level_id" class="form-control">
                            </div>
                            <div class="form-group">
                                <label for="exampleSelect1">Status</label>
                                <select class="form-control" name="job_level_status" id="edit_job_level_status">
                                    <option value="">Pilih Status</option>
                                    <option value="1">Aktif</option>
                                    <option value="0">Tidak Aktif</option>
                                </select>
        ...
        <script>
            $('#btn-add-save').click(function (e) {
                ...
                var add_job_level_name   = $('#add_job_level_name').val();
                var add_job_level_status = $('#add_job_level_status').val();

                if (add_job_level_name != '') {
                    if (add_job_level_status != '') {

                        var postData = new FormData(this.form);                        
                        $.ajax({
                            url : "<?php echo base_url('pengaturan/job_level_add_data'); ?>",
                            type: "POST",
                            data: postData,
                            processData: false,
                            contentType: false,
                            success: function(data, textStatus, jqXHR){
                                ...
                                if(data.status == 'TRUE'){ 
                                    $('#cover-spin').hide(0);
                                    Swal.fire("Berhasil", "Tambah Level Pekerjaan Berhasil", "success");
                                    setTimeout(function() {
                                        location.reload();
                                    }, 2000);
                                    ...
                                    $('#cover-spin').hide(0);
                                    Swal.fire("Gagal", "Tambah Level Pekerjaan Gagal", "error");
                        ...
                        $('#cover-spin').hide(0);
                        Swal.fire("Gagal!", "Status Level Pekerjaan Tidak Boleh Kosong!", "error");
                    }
                } else {
                    $('#cover-spin').hide(0);
                    Swal.fire("Gagal!", "Nama Level Pekerjaan Tidak Boleh Kosong!", "error");
                }
            ...
            function edit(id) {
                $.ajax({    
                    type: "GET",
                    url: "<?php echo base_url('pengaturan/job_level_get_edit_data/')?>" + id,             
                    dataType: "JSON",   
                    success: function(response){     
                        console.log(response);               
                        $('#modal-edit').modal('show');
                        $('#edit_job_level_id').val(response[0].master_job_level_id);
                        $('#edit_job_level_name').val(response[0].master_job_level_name);
                        $('#edit_job_level_status').val(response[0].master_job_level_status);         
                    }
                });
            }
            ...
            $('#btn-edit-save').click(function (e) {
                ...
                var edit_job_level_name      = $('#edit_job_level_name').val();
                var edit_job_level_status    = $('#edit_job_level_status').val();
                var edit_job_level_id        = $('#edit_job_level_id').val();

                if (edit_job_level_name != '') {
                    if (edit_job_level_status != '') {
                        if (edit_job_level_id != '') {

                            var postData = new FormData(this.form);                        
                            $.ajax({
                                url : "<?php echo base_url('pengaturan/job_level_update_data'); ?>",
                                type: "POST",
                                data: postData,
                                processData: false,
                                contentType: false,
                                success: function(data, textStatus, jqXHR){
                                    ...
                                    if(data.status == 'TRUE'){ 
                                        $('#cover-spin').hide(0);
                                        Swal.fire("Berhasil", "Update Level Pekerjaan Berhasil", "success");
                                        setTimeout(function() {
                                            location.reload();
                                        }, 2000);
                                        ...
                                        $('#cover-spin').hide(0);
                                        Swal.fire("Gagal", "Update Level Pekerjaan Gagal", "error");
            ...
            function delete_data(id) {
                Swal.fire({
                    ...
                    if (result.value) {    
                        ...
                        $.ajax({
                            type: "GET",
                            url: "<?php echo base_url('pengaturan/job_level_delete_data/')?>" + id,             
                            dataType: "JSON",   
                            success: function(data, textStatus, jqXHR){
                                ...
                                if(data.status == 'TRUE'){ 
                                    ...
                                    $('#cover-spin').hide(0);
                                    Swal.fire("Gagal", "Hapus Level Pekerjaan Gagal", "error");
            ...
            function getData() {
                return new Promise((resolve) => {
                    $.ajax({    
                        type: "GET",
                        url: "<?php echo base_url('pengaturan/job_level_get_data'); ?>",             
                        dataType: "JSON",   
                        success: function(response){                    
                            //    console.log(response);     
                            resolve(response)        
