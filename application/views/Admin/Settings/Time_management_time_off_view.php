	...
	<body id="kt_body" style="background-image: url(<?php echo base_url(); ?>assets/template/metronic/media/bg/header.jpg)" class="quick-panel-right demo-panel-right offcanvas-right header-fixed subheader-enabled page-loading">
                                ...
                                <div class="card card-custom">
                                    <div class="card-header flex-wrap border-0 pt-6 pb-0">
                                        <div class="card-title">
                                            <h3 class="card-label">Daftar Cuti
        ...
        <div class="modal fade" id="exampleModalLong" data-backdrop="static" tabindex="-1" role="dialog" aria-labelledby="staticBackdrop" aria-hidden="true">
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    ...
                    <form method="POST" action="" id="form-add-time_off">
                        <input type="hidden" name="<?php echo $this->security->get_csrf_token_name(); ?>" value="<?php echo $this->security->get_csrf_hash(); ?>" id="csrf_token" />
                        <div class="modal-body">
                            <div class="form-group">
                                <label>Nama Tipe Cuti</label>
                                <input type="text" name="time_off_name" id="add_time_off_name" class="form-control">
                            </div>
                            <div class="form-group">
                                <label for="exampleSelect1">Status</label>
                                <select class="form-control" name="time_off_status" id="add_time_off_status">
        ...
        <div class="modal fade" id="modal-edit" data-backdrop="static" tabindex="-1" role="dialog" aria-labelledby="staticBackdrop" aria-hidden="true">
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    ...
                    <form method="POST" action="" id="form-edit-time-off">
                        <input type="hidden" name="<?php echo $this->security->get_csrf_token_name(); ?>" value="<?php echo $this->security->get_csrf_hash(); ?>" id="csrf_token" />
                        <div class="modal-body">
                            <div class="form-group">
                                <label>Nama Tipe Cuti</label>
                                <input type="text" name="time_off_name" id="edit_time_off_name" class="form-control">
                                <input type="hidden" name="time_off_id" id="edit_time_off_id" class="form-control">
                            </div>
                            <div class="form-group">
                                <label for="exampleSelect1">Status</label>
                                <select class="form-control" name="time_off_status" id="edit_time_off_status">
                                    <option value="">Pilih Status</option>
                                    <option value="1">Aktif</option>
                                    <option value="0">Tidak Aktif</option>
                                </select>
        ...
        <script>
            $('#btn-add-save').click(function (e) {
                ...
                var add_time_off_name   = $('#add_time_off_name').val();
                var add_time_off_status = $('#add_time_off_status').val();

                if (add_time_off_name != '') {
                    if (add_time_off_status != '') {

                        var postData = new FormData(this.form);                        
                        $.ajax({
                            url : "<?php echo base_url('pengaturan/time_off_add_data'); ?>",
                            type: "POST",
                            data: postData,
                            processData: false,
                            contentType: false,
                            success: function(data, textStatus, jqXHR){
                                ...
                                if(data.status == 'TRUE'){ 
                                    $('#cover-spin').hide(0);
                                    Swal.fire("Berhasil", "Tambah Tipe Cuti Berhasil", "success");
                                    setTimeout(function() {
                                        location.reload();
                                    }, 2000);
                                    ...
                                    $('#cover-spin').hide(0);
                                    Swal.fire("Gagal", "Tambah Tipe Cuti Gagal", "error");
                        ...
                        $('#cover-spin').hide(0);
                        Swal.fire("Gagal!", "Status Tipe Cuti Tidak Boleh Kosong!", "error");
                    }
                } else {
                    $('#cover-spin').hide(0);
                    Swal.fire("Gagal!", "Nama Tipe Cuti Tidak Boleh Kosong!", "error");
                }
            ...
            function edit(id) {
                $.ajax({    
                    type: "GET",
                    url: "<?php echo base_url('pengaturan/time_off_get_edit_data/')?>" + id,             
                    dataType: "JSON",   
                    success: function(response){     
                        console.log(response);               
                        $('#modal-edit').modal('show');
                        $('#edit_time_off_id').val(response[0].master_time_off_id);
                        $('#edit_time_off_name').val(response[0].master_time_off_name);
                        $('#edit_time_off_status').val(response[0].master_time_off_status);         
                    }
                });
            }
            ...
            $('#btn-edit-save').click(function (e) {
                ...
                var edit_time_off_name      = $('#edit_time_off_name').val();
                var edit_time_off_status    = $('#edit_time_off_status').val();
                var edit_time_off_id        = $('#edit_time_off_id').val();

                if (edit_time_off_name != '') {
                    if (edit_time_off_status != '') {
                        if (edit_time_off_id != '') {

                            var postData = new FormData(this.form);                        
                            $.ajax({
                                url : "<?php echo base_url('pengaturan/time_off_update_data'); ?>",
                                type: "POST",
                                data: postData,
                                processData: false,
                                contentType: false,
                                success: function(data, textStatus, jqXHR){
                                    ...
                                    if(data.status == 'TRUE'){ 
                                        $('#cover-spin').hide(0);
                                        Swal.fire("Berhasil", "Update Tipe Cuti Berhasil", "success");
                                        setTimeout(function() {
                                            location.reload();
                                        }, 2000);
                                        ...
                                        $('#cover-spin').hide(0);
                                        Swal.fire("Gagal", "Update Tipe Cuti Gagal", "error");
            ...
            function delete_data(id) {
                Swal.fire({
                    ...
                    if (result.value) {     
                        ...
                        $.ajax({
                            type: "GET",
                            url: "<?php echo base_url('pengaturan/time_off_delete_data/')?>" + id,             
                            dataType: "JSON",   
                            success: function(data, textStatus, jqXHR){
                                ...
                                if(data.status == 'TRUE'){ 
                                    ...
                                    $('#cover-spin').hide(0);
                                    Swal.fire("Gagal", "Hapus Tipe Cuti Gagal", "error");
            ...
            function getData() {
                return new Promise((resolve) => {
                    $.ajax({    
                        type: "GET",
                        url: "<?php echo base_url('pengaturan/time_off_get_data'); ?>",             
                        dataType: "JSON",   
                        success: function(response){                    
                            //    console.log(response);     
                            resolve(response)        
                ...
                var datatable = $('#kt_datatable').KTDatatable({
                    ...
                    columns: [{
                        ...
                        template: function(row) {
                            ...
                            return '<span class="label font-weight-bold label-lg ' + status[row.time_off_status].class + ' label-inline">' + status[row.time_off_status].title + '</span>';
                        },
                    }, {
                        field: 'Actions',
                        title: 'Aksi',
                        ...
                        template: function(row) {
                            return '\
                                    <a href="javascript:;" onclick="edit('+row.time_off_id+')" class="btn btn-sm btn-clean btn-icon mr-2" title="Edit details">\
                                        <span class="svg-icon svg-icon-md">\
                                            <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="24px" height="24px" viewBox="0 0 24 24" version="1.1">\
                                                <g stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">\
                                                    <rect x="0" y="0" width="24" height="24"/>\
                                    ...
                                    <a href="javascript:;" onclick="delete_data('+row.time_off_id+')" class="btn btn-sm btn-clean btn-icon" title="Delete">\
                                        <span class="svg-icon svg-icon-md">\
                                            <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="24px" height="24px" viewBox="0 0 24 24" version="1.1">\
                                                <g stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">\
                                                    <rect x="0" y="0" width="24" height="24"/>\
